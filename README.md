# What I'd Build for Figma — the LinkedIn Intent Miner
...
> **Quick start (zero credentials):** `cd figma-intent-tracker && pip install -r requirements.txt && MODE=demo python pipeline.py` — a full run on labeled synthetic fixtures. `python -m pytest -q` runs the 90 tests. Full instructions in §10.
...
![AI Sales Engineering @ Figma — the full map](figma-intent-tracker/docs/img/gtm-map.png)
...
All paths below live under `figma-intent-tracker/`.
